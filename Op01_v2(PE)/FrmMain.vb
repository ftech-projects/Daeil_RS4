...
Public Class FrmMain
    ...
    Private Sub SetFrameBarcodeNg(ByVal reason As String, ByVal scanBarcode As String, Optional ByVal dbBarcode As String = "")
        srclbDataFrameBarcode.Text = scanBarcode
        srclbDecFrameBarcode.Text = "NG"
        srclbDecFrameBarcode.BackColor = Color.FromArgb(127, 29, 29)
        srclbDecFrameBarcode.ForeColor = Color.FromArgb(239, 68, 68)
        If dbBarcode <> "" Then
            WriteTxtMessage("[FRAME] " & reason & " | DB=" & dbBarcode)
        Else
            WriteTxtMessage("[FRAME] " & reason)
        End If
        NG()
    End Sub

    ''' <summary>Play Table_Fram_Inspection 전체 조회 후 일치 행 검증 — OK 시 True</summary>
    Private Function TryApplyFrameBarcodeScan(ByVal scanData As String) As Boolean
        Dim trimmedScan As String = Trim(If(scanData, ""))

        Dim row As FrameInspectionDb.FrameInspectionRow = FrameInspectionDb.QueryFrameInspection(trimmedScan)

        If row.QueryError <> "" Then
            SetFrameBarcodeNg("DB 조회 실패 — " & row.QueryError, trimmedScan)
            Return False
        End If

        If Not row.Found Then
            SetFrameBarcodeNg("Play 실적 없음 — " & trimmedScan, trimmedScan)
            Return False
        End If

        srclbDataFrameBarcode.Text = trimmedScan

        If Not row.IsLatest Then
            SetFrameBarcodeNg("최신 바코드 불일치 — 스캔=" & trimmedScan, trimmedScan, row.FrameBarcode)
            Return False
        End If

        If String.Compare(row.Decision, "OK", StringComparison.OrdinalIgnoreCase) <> 0 Then
            SetFrameBarcodeNg("Play 판정 NG — Decision=" & row.Decision, trimmedScan, row.FrameBarcode)
            Return False
        End If

        If InStr(1, row.FrameBarcode, TargetFrameBarcode) = 0 Then
            SetFrameBarcodeNg("품번 목표 불일치 — Target=" & TargetFrameBarcode, trimmedScan, row.FrameBarcode)
            Return False
        End If

        srclbDecFrameBarcode.Text = "OK"
        srclbDecFrameBarcode.BackColor = Color.Blue
        srclbDecFrameBarcode.ForeColor = Color.White
        WriteTxtMessage("[FRAME] OK — 스캔=" & trimmedScan & " | DB=" & row.FrameBarcode)
        DingDOng()
        Return True
    End Function
    ...
    Private Sub Serial_Scanner_DataReceived(ByVal sender As Object, ByVal e As System.IO.Ports.SerialDataReceivedEventArgs) Handles Serial_Scanner.DataReceived
        ...
        If wStep = 3 Then
            ...
            Dim scanHandled As Boolean = False
            ...
            If (srclbDecFrameBarcode.Text <> "NA") AndAlso (srclbDecFrameBarcode.Text <> "OK") AndAlso
                InStr(1, ScanData, TargetFrameBarcode) <> 0 Then
                If TryApplyFrameBarcodeScan(ScanData) Then
                    scanHandled = True
                Else
                    scanHandled = True
                End If
